...
function solveWorksheet($input) {
    ...
    foreach ($problems as $problem) {
        ...
        for ($col = $problem['end']; $col >= $problem['start']; $col--) {
            $columnDigits = '';
            
            foreach ($numberRows as $row) {
                if ($col < strlen($row) && $row[$col] !== ' ') {
                    $columnDigits .= $row[$col];
                }
            }
            
            if (!empty(trim($columnDigits))) {
                $numbers[] = (int)trim($columnDigits);
            }
        }
...
echo "Grand total: " . solveWorksheet($daySixTwoInput) . "\n";
